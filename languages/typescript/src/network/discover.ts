...
export type DiscoveryError =
  // No bundled trust anchor matches `target` at all, so there was nothing
  ...
  constructor(detail: DiscoveryError) {
    const message =
      detail.kind === 'no-candidates'
        ? `no bundled trust anchor matches target network ${detail.target}`
 ...
 * reading `getBundledTrustAnchors()`, so tests can supply a mock server's
 * own key instead of needing to forge a signature for a real bundled entry.
 */
 ...
 * `getBundledTrustAnchors()`'s own `server_url`/`seed_nodes` fields for
 ...
 * Entries are tried in `getBundledTrustAnchors()`'s order, and each
